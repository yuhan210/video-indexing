    ...
    for fid in xrange(len(_vgg_data)):
        ...
        if len(msr_data) > 0:
            for msr_idx, w in enumerate(msr_data['words']['text']):
                ...
                if w not in stop_words and prob > -5 and msr_idx < 30:
                    msr_ws += [w]
...
if __name__ == "__main__":
    ...
    for vid, video_name in enumerate(videos):
        ...
        _vgg_data, dummy = load_video_recog('/mnt/tags/vgg-classify-all', video_name)
        _fei_caption_data, dummy = load_video_caption('/mnt/tags/fei-caption-all', video_name)
        _msr_cap_data, dummy = load_video_msr_caption('/mnt/tags/msr-caption-all', video_name)
        _rcnn_data, dummy = load_video_rcnn('/mnt/tags/rcnn-info-all', video_name)

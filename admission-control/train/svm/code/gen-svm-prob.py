...
if __name__ == "__main__":
    ...
    for line in lines: 
        ...
        feature_tuple = [ enctype, w, h, encsize, mvsize, meanmv, maxmv, minmv, sobel, illu, framediff, phash, colorhist, siftmatch, surfmatch] 
        bad_feature = False 
        for f in feature_tuple:
            if float(f) < 0:
                ...
                break
        if not bad_feature:
            train_data[video_name][label] += [[ enctype, w, h, encsize, mvsize, meanmv, maxmv, minmv, sobel, illu, framediff, phash, colorhist, siftmatch, surfmatch] ] 
        

    print 'Finish parsing train.data'
    
    outputfh = open('svm_train_half.data', 'w')

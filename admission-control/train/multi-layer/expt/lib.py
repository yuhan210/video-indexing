...
def load_all_data_second_layer(INPUT_TRAIN_DATA = "/home/t-yuche/admission-control/train/train.data"):
    ...
    for line in lines: 
        ...
        if prev_fid == 0:
            illu_diff = 0
            sobel_diff = 0
            prev_illu = float(illu)
            prev_sobel = float(sobel)
        else:
            illu_diff = abs(float(illu) - float(prev_illu))
            sobel_diff = float(sobel) - float(prev_sobel)
        ...
        #feature_tuple = [int(enctype), encsize_norm, float(mvsize)/(float(w) * float(h)), float(meanmv), float(maxmv), float(sobel), float(illu), float(framediff), float(phash) , float(colorhist), float(surfmatch), int(illu_diff), int(fid)] 
        feature_tuple = [float(framediff), float(phash) , float(colorhist), float(surfmatch), int(fid)] 

        train_data[video_name][label] += [feature_tuple]
        #train_data[video_name]['fid'] = int(fid)

    return train_data

def load_all_data_first_layer(INPUT_TRAIN_DATA = "/home/t-yuche/admission-control/train/train.data"):
    ...
    for line in lines: 
        ...
        feature_tuple = [int(enctype), float(encsize), float(mvsize), float(meanmv), float(maxmv), int(dist_from_prevfid)] 

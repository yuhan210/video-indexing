...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 3:
        print 'Usage', sys.argv[0], 'frame_folder(-/frame) sample_freq'
        exit(-1)
    ...
    for v in os.listdir(video_folder):
        ...
        for idx, f in enumerate(os.listdir(video_path)):
            if idx % sample_freq == 0:
                fh_task_out.write(os.path.abspath(os.path.join(video_path, f)) + '\n')
        
        fh_task_out.close()	
        # append to run script
        fh_out.write('python /home/t-yuche/neuraltalk/python_features/extract_features.py  --gpu --caffe ~/caffe/ --model_def /home/t-yuche/neuraltalk/python_features/deploy_features.prototxt --model /home/t-yuche/caffe/models/vgg_ilsvrc_16/VGG_ILSVRC_16_layers.caffemodel --files ' +  task_filename + ' --out /mnt/tags/caption/' + v + '.pickle\n')

        fh_out.write('python /home/t-yuche/neuraltalk/predict_on_images.py /home/t-yuche/neuraltalk/models/flickr8k_cnn_lstm_v1.p -r ' + video_path +  ' -t ' + task_filename  + ' -f  /mnt/tags/caption/' + v + '.pickle ' + '-of  /mnt/tags/caption/' + v + '_5_caption.json &\n\n')

...
parser.add_argument('--features',
                    help='path to the relu 7 feature pickle file')
...
def feature_classify(in_data, net):

    net.blobs['fc7'].data[...] = in_data
    out = net.forward(start='fc7')
    print out
    predictions = out[net.outputs[0]]
    all_labels = []
    all_confs = []
...
def batch_predict_feature(filenames, net, labels, feature_path):
    #print net.blobs 
    ...
    Nf, D = features.shape # D = 4096
    print Nf, D
    
    N, DC, DH, DW = net.blobs[net.inputs[0]].data.shape
    print N
    # do it in batch  
    for i in range(0, Nf, N):
        tic = time.time()
        in_data = np.zeros((N, D), dtype=np.float32)
        
        batch_range = range(i, min(i+N, Nf))
        batch_filenames = [filenames[j] for j in batch_range] 
        Nb = len(batch_range)
        
        batch_features = features[ i: min(i+N, Nf) , :]
        # insert into correct place
        in_data[0:len(batch_range), :] = batch_features
        
        (ps,cs) = feature_classify(in_data, net)
        print ps, cs
        toc = time.time()
...
feature_allpreds = batch_predict_feature(filenames, net, labels, args.features)
print 'feature based done'

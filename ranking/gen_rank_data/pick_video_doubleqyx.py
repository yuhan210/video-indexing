...
if __name__ == "__main__":
    ...
    for sid, ss in enumerate(start_fid_set):
        ...
        #for query_str in open('./double_query_yx').readlines():
        for query_str in ['cat kitchen']:
            ...
            for vid, video_name in enumerate(videos): 
                ...
                vis_agg_info = {'vis_obj_count' : vis_obj_count, 'max_dwell_time': max_dwell_time, 'max_w_h': max_w_h}
            ...
            last_resort_videos = []
            for video_name in videos:
                if video_info[video_name]['gt_match_count'] == target_count and video_info[video_name]['subsample_match_count'] == target_count and video_info[video_name]['optimal_match_count'] == target_count and (video_info[video_name]['vis_agg_info']['max_dwell_time'] > 0):
                ...
                #elif video_info[video_name]['gt_match_count'] > 0 and video_info[video_name]['subsample_match_count'] > 0:
                #    subsample_better_videos += [video_info[video_name]] 
                #elif video_info[video_name]['gt_match_count'] > 0 and video_info[video_name]['optimal_match_count'] > 0:
            ...
            for v in other_videonames: 
                if len(all_videos) >= 10 or counter == 2:
                    break
                if v['video_name'] not in [x['video_name'] for x in all_videos]:
                    all_videos += [v]

            other_videonames = sorted(other_videos, key = lambda x: x['vis_agg_info']['max_dwell_time'], reverse = True)
            counter = 0
            for v in other_videonames: 
                if len(all_videos) >= 10 or counter == 2:
                    break
            ...
            other_videonames = sorted(other_videos, key = lambda x: x['vis_agg_info']['max_w_h'], reverse = True)
            counter = 0
            for v in other_videonames: 
                ...
                if v['video_name'] not in [x['video_name'] for x in all_videos]:
                    all_videos += [v]
            print '------ other videos:', len(all_videos)
            ...
            tolerate_videonames = sorted(tolerate_videos, key = lambda x: x['vis_agg_info']['max_w_h'], reverse = True)
            for v in rest_videonames: 
                if len(all_videos) >= 10:
                    break
                if v['video_name'] not in [x['video_name'] for x in all_videos]:

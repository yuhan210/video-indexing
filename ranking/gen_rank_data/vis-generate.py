...
def process_visual_hint(chunk, w, h):
    ...
    for CLASS in CLASSES:
        obj_trace[CLASS] = {'trace': [], 'dwell_time': 0, 'obj_num': 0, 'moving_speed': -1, 'ave_obj_size': [0,0], 'max_obj_size': [0,0] }
        ...
        for idx, frame_info in enumerate(chunk):
            ...
            for obj in frame_info['pred']:
                if obj['score'] >= 0.5 and obj['class'] == CLASS:
                    ...
                    bbox = obj['bbox']
                    bbox_w = (bbox[2] - bbox[0])/ (w * 1.0)
                    bbox_h = (bbox[3] - bbox[1])/ (h * 1.0)
            ...
            elif target_bbox != None and len(objs) > 0:
                #print 'objs:', objs
                nbbx = get_nearest_bbox(target_bbox, objs, w,h)
                #print 'trace:', target_bbox, nbbx 
                d = get_center_dist(target_bbox, nbbx, w, h)
                speeds += [d/((idx - prev_fid) * 1.0)] 
                target_bbx = list(nbbx)
                prev_fid = idx 
...
if __name__ == "__main__":
    ...
    for idx, ss in enumerate(start_fid_set):
        outpath = './vis-log/all_vis_' + str(idx) + '.pickle'
